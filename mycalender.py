import calendar
from datetime import datetime


class MyCalendar(calendar.LocaleHTMLCalendar):
    def formatmonth(self, theyear, themonth, withyear=True):
        # 親クラスほとんど継承 (クラスだけ違う)
        v = []
        a = v.append
        # 変更点
        a('<table class="table table-bordered table-sm" style="table-layout: fixed;">')
        # 変更点ここまで
        a('\n')
        a(self.formatmonthname(theyear, themonth, withyear=withyear))
        a('\n')
        a(self.formatweekheader())
        a('\n')
        for week in self.monthdays2calendar(theyear, themonth):
            # ここも違う。年月日全て渡すようにする(後述)
            a(self.my_formatweek(week, theyear, themonth))
            a('\n')
        a('</table><br>')
        a('\n')
        return ''.join(v)

    def my_formatweek(self, theweek, theyear, themonth):
        '''
        オーバーライド (引数を変えるのはPythonでは多分非推奨)
        引数で year と month を渡すようにした。
        '''
        s = ''.join(self.my_formatday(d, theyear, themonth) for (d, wd) in theweek)
        return '<tr>%s</tr>' % s

    def my_formatday(self, day, theyear, themonth):
        '''
        オーバーライド (引数を変えるのはPythonでは多分非推奨)
        引数で year と month を渡すようにした。
        '''
        if day == 0:
            # 空白
            return '<td style="background-color: #eeeeee">&nbsp;</td>'
        else:
            html = '<td class="text-center {highlight}"><a href="{url}" style="color:{text}">{day}</a></td>'
            text = 'blue'
            highlight = ''
            # もし予定があるなら強調
            date = datetime(year=theyear, month=themonth, day=day)
            date_str = date.strftime('%Y%m%d')
            if date_str in self.linked_date:
                # 終了した予定
                if self.linked_date[date_str]:
                    highlight = 'bg-success'
                    text = 'white'
                # 過去の予定
                elif date < datetime.now():
                    highlight = 'bg-secondary'
                    text = 'white'
                # これからの予定
                else:
                    highlight = 'bg-warning'
            # urlをtodo/{username}/year/month/day に変更
            return html.format(
                url='/todo/{}/{}/{}/{}'.format(self.username, theyear, themonth, day),
                text=text,
                day=day,
                highlight=highlight)

    def __init__(self, username, linked_date: dict):
        calendar.LocaleHTMLCalendar.__init__(self,
                                             firstweekday=6,
                                             locale='ja_jp')
        # 何か予定がある日付はリンクする
        self.username = username
        # dict{'datetime': done}
        self.linked_date = linked_date

    pass
